// Ruler generator: an engraved ruler for laser cutting, in millimeters or
// inches. Ticks and numbers are engraved in one color, the outline is cut
// in another.
'use strict';
...
// baseY, digit height = size.
function digitsPath(num, cx, baseY, size) {
  ...
  for (const ch of str) {
    (DIGITS[ch] || []).forEach(pl => {
      pl.forEach((p, i) => {
        d += (i === 0 ? 'M' : 'L') + fmtMm(x + p[0] * size) + ' ' + fmtMm(baseY - size + p[1] * size);
      });
    });
    x += DIGIT_ADV * size;
  }
...
// Ticks as [position in mm, length in mm, label or null]; the scale
// includes a labeled 0 at its start.
// mm: three levels (1 mm / 5 mm / 1 cm), numbers on centimeters; `fine`
// adds half-millimeter ticks of the tiny length.
// inch decimal: 1/10, 1/2 and whole inch levels.
// inch binary: 1/16 .. 1 inch, the in-between levels interpolated between
// the smallest and middle tick lengths; `fine` adds 1/32 ticks of the
// tiny length.
function rulerTicks(params) {
  const t1 = params.tick1, t5 = params.tick5, t10 = params.tick10;
  const t0 = params.tick05 > 0 ? params.tick05 : t1 * 0.6;
  const ticks = [];
  const n = Math.round(params.length);
  if (params.units === 'inch') {
    if (params.divisions === 'binary') {
      const t4 = t1 + (t5 - t1) * 2 / 3;
      const t8 = t1 + (t5 - t1) / 3;
      const d = params.fine ? 32 : 16;
      for (let i = 0; i <= n * d; i++) {
      ...
      const d = params.fine ? 20 : 10; // fine halves the tenths
      for (let i = 0; i <= n * d; i++) {
        const k = i * (20 / d);
...
function generateRuler(params) {
  ...
  if (!(params.length > 0 && H > 0 && t1 > 0 && t5 > 0 && t10 > 0)) {
  ...
  const scaleW = params.length * (inch ? INCH : 1);
  const W = mL + scaleW + mR;
  const units = Math.round(params.length);
  if ((inch ? units * 32 : units * 2) > 8000) {
    return { svg: '', warnings: ['The ruler is too long.'] };
  }
  if (Math.max(t1, t5, t10) >= H) {
    warnings.push('Ticks are longer than the ruler height.');
  }
  if (labels && t10 + labelSize + 2 > H) {
    warnings.push('Numbers do not fit above the longest ticks — lower them or raise the height.');
  }
  const step = inch
    ? (params.divisions === 'binary' ? INCH / (params.fine ? 32 : 16) : INCH / 10)
    : (params.fine ? 0.5 : 1);
  if (step - stroke < 0.2) {
    warnings.push('Ticks are nearly as wide as their spacing — reduce the line width.');
  }

  const m = 5;
  let marks = '';
  rulerTicks(params).forEach(([x, len, num]) => {
    // margins shift the scale off the ends; the inverse ruler runs its
    // scale from right to left (zero at the right margin)
    const px = inverse ? mL + scaleW - x : mL + x;
    // a tick right on a cut edge would be cut away anyway — skip it
    if (px > 0.01 && px < W - 0.01) {
      marks += 'M' + fmtMm(m + px) + ' ' + fmtMm(m + H) + 'L' + fmtMm(m + px) + ' ' + fmtMm(m + H - len);
    }
    if (labels && num !== null) {
      ...
      marks += digitsPath(num, m + lx, m + H - t10 - 1, labelSize);
    }
  });
